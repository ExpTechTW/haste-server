...
- **儲存前先確認** —— 一個對話框收下標題（15 字以內，選填）與保存期限，因為寫入即鎖死，這些之後都改不了。
...
| 動作 | 方法 |
| ---- | ---- |
| 儲存 | `⌘/Ctrl + S`，或按儲存 —— 會先跳出確認對話框 |
| 命名 | 對話框裡的標題欄，15 字以內，選填 |
| 設定期限 | 對話框裡的時鐘選單 —— 1h ~ 30d，或不限制 |
...
標題是選填的，最多 15 個字元（不是 bytes —— 15 個中文字就是 15 個字元）。設了標題，連結預覽與瀏覽器分頁就顯示它，取代自動產生的 `Python · 410 字元`；沒設就照舊。標題和內容一樣不可變更，且會拒收控制字元與雙向覆寫字元 —— 那些在別人的聊天視窗裡是偽裝工具，不是排版工具。
